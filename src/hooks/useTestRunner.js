...
export const useTestRunner = () => {
  ...
  const runScenario = useCallback(
    async (scenarioName, cpId) => {
      const currentConnectors = cpState.chargePoints[cpId]?.connectors || [];
      if (!isConnected) {
        actions.addLog(cpId, {
          direction: "ERROR",
          text: `Scenario failed: Not connected to CSMS.`,
        });
        return;
      }
      ...
      if (!scenario) {
        actions.addLog(cpId, {
          direction: "ERROR",
          text: `Scenario "${scenarioName}" not found.`,
        });
        return;
      }
      ...
      actions.addLog(cpId, {
        direction: "SYSTEM",
        text: `--- Starting Scenario: ${scenario.name} ---`,
      });
      let passed = true;
      ...
      for (const step of scenario.steps) {
        ...
        try {
          ...
          if (
            ...
            actions.addLog(cpId, {
              direction: "SYSTEM",
              text: `  -> Injecting Transaction ID: ${currentTransactionId}`,
            });
          } else if (
            step.action === "StatusNotification" ||
            step.action === "Heartbeat" ||
            step.action === "Authorize"
          ) {
            // Log when these non-transactional messages are processed
            actions.addLog(cpId, {
              direction: "SYSTEM",
              text: `  -> Sending Non-Transactional Message: ${step.action}`,
            });
          }

          if (
            step.action === "RemoteStartTransaction" ||
            step.action === "RemoteStopTransaction" ||
            step.action === "GetConfiguration" ||
            step.action === "ChangeConfiguration"
          ) {
            const endpoint = step.action
              .toLowerCase()
              .replace("transaction", ""); // e.g., 'remotestart' or 'remotestop'

            // 2. Construct the dynamic URL
            const remoteApiUrl = `${apiUrl}/adminApi/chargers/${cpId}/${endpoint}`;
            console.log("remoteApiUrl", remoteApiUrl);
            actions.addLog(cpId, {
              direction: "SYSTEM",
              text: `  -> Calling API for ${step.action}`,
            });
            ...
            conf = {
              payload: {
                status:
                  apiResponse.status === 200 || apiResponse.status === 202
                    ? "Accepted"
                    : "Rejected",
              },
            };
          } else if (
            step.action === "StartTransaction" ||
            step.action === "StopTransaction" ||
            step.action === "MeterValues" ||
            step.action === "StatusNotification" ||
            step.action === "Heartbeat"
          ...
          if (
            ...
            actions.addLog(cpId, {
              direction: "SYSTEM",
              text: `  -> Transaction ${currentTransactionId} started on Connector ${connectorIdToUpdate}.`,
            });
          }
          ...
          if (
            ...
            actions.addLog(cpId, {
              direction: "SYSTEM",
              text: `Transaction ${requestPayload.transactionId} officially ended. Status reset to Available.`,
            });
          ...
          if (
            conf &&
            conf.payload.status &&
            conf.payload.status.toLowerCase() === "rejected"
          ) {
            actions.addLog(cpId, {
              direction: "ERROR",
              text: `${step.action} REJECTED by CSMS. Stopping scenario.`,
            });
            passed = false;
            break;
          }
          ...
          if (
            ...
            actions.addLog(cpId, {
              direction: "ERROR",
              text: `${step.action} API FAILED (Status ${apiResponse.status}). Stopping scenario.`,
            });
            passed = false;
            break;
          }
          ...
          actions.addLog(cpId, {
            direction: "ERROR",
            text: `Scenario failed at ${step.action}: ${error.message}`,
          });
          passed = false;
          break;
        }
      ...
      actions.addLog(cpId, {
        direction: "SYSTEM",
        text: `--- Scenario ${resultText}: ${scenario.name} ---`,
      });
    },
    [
